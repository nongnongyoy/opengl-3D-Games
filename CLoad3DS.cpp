...
int gotHeight(BYTE *pHeightMap, int X, int Y)				// This Returns The Height From A Height Map Index
{
...
void drawModel(t3DModel Model, BYTE *pHeightMap, bool touming,bool outTex, int x1, int y1, int z1, float time)
{
	...
	for(int i = 0; i < Model.numOfObjects; i++)
	{
		...
		for(int j = 0; j < pObject->numOfFaces; j++)
		{
			for(int whichVertex = 0; whichVertex < 3; whichVertex++)
			{
				...
				glVertex3f(time*pObject->pVerts[ index ].x+x1, time*pObject->pVerts[ index ].y+y1, time*pObject->pVerts[ index ].z+z1);
			}
